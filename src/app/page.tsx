    ...
    <AppShell>
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <Link href="/create" className="btn-primary w-full sm:w-auto justify-center">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="card-hover p-4 sm:p-6 group">
            <div className="flex items-center gap-3 sm:gap-4">
              <div className="p-2.5 sm:p-3 bg-gradient-to-br from-red-100 to-red-50 rounded-lg group-hover:shadow-lg transition-all">
                <Receipt className="w-5 h-5 sm:w-6 sm:h-6 text-red-600" />
              </div>
              <div>
                <p className="text-2xl sm:text-3xl font-bold text-gray-900">{stats?.invoiceCount || 0}</p>
                <p className="text-xs sm:text-sm text-gray-600 font-medium">Invoices</p>
              </div>
            </div>
          </div>
          ...
          <div className="card-hover p-4 sm:p-6 group">
            <div className="flex items-center gap-3 sm:gap-4">
              <div className="p-2.5 sm:p-3 bg-gradient-to-br from-blue-100 to-blue-50 rounded-lg group-hover:shadow-lg transition-all">
                <FileText className="w-5 h-5 sm:w-6 sm:h-6 text-blue-600" />
              </div>
              <div className="min-w-0">
                <p className="text-2xl sm:text-3xl font-bold text-gray-900">{stats?.quotationCount || 0}</p>
                <p className="text-xs sm:text-sm text-gray-600 font-medium">Quotations</p>
              </div>
            </div>
          </div>
          ...
          <div className="card-hover p-4 sm:p-6 group">
            <div className="flex items-center gap-3 sm:gap-4">
              <div className="p-2.5 sm:p-3 bg-gradient-to-br from-amber-100 to-amber-50 rounded-lg group-hover:shadow-lg transition-all">
                <Truck className="w-5 h-5 sm:w-6 sm:h-6 text-amber-600" />
              </div>
              <div className="min-w-0">
                <p className="text-2xl sm:text-3xl font-bold text-gray-900">{stats?.waybillCount || 0}</p>
                <p className="text-xs sm:text-sm text-gray-600 font-medium">Waybills</p>
              </div>
            </div>
          </div>

          <div className="card-hover p-4 sm:p-6 group col-span-2 lg:col-span-1">
            <div className="flex items-center gap-3 sm:gap-4">
              <div className="p-2.5 sm:p-3 bg-gradient-to-br from-green-100 to-green-50 rounded-lg group-hover:shadow-lg transition-all">
        ...
        <div className="card p-4 sm:p-6">
          <div className="flex items-center justify-between mb-4 gap-2">
            ...
            <Link href="/documents" className="text-sm text-red-600 hover:text-red-700 font-medium transition-colors shrink-0">
              View all →
            </Link>
          </div>
            ...
            <div className="space-y-2">
              {recentDocs.map((doc) => (
                <Link
                  ...
                  className="flex items-center justify-between gap-3 p-3 sm:p-4 rounded-lg hover:bg-gray-50 transition-all duration-200 border border-transparent hover:border-gray-200"
                >
                  <div className="flex items-center gap-3 min-w-0">
                    <div
                      className={`p-2 rounded-lg shrink-0 ${doc.type === 'invoice'
                          ? 'bg-red-100'
                          : doc.type === 'quotation'
                            ? 'bg-blue-100'
                            : 'bg-amber-100'
                    ...
                    <div className="min-w-0">
                      <p className="font-semibold text-sm text-gray-900 truncate">{doc.serialNumber}</p>
                      <p className="text-xs text-gray-500 truncate">{doc.customer.name}</p>
                    </div>
                  </div>
